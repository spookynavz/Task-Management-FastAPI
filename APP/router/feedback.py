...
def add_feedback(feedback: FeedbackCreate, db: Session = Depends(get_db), current_user=Depends(manager_required)):
...
def retrieve_feedback(task_id: int, db: Session = Depends(get_db), current_user=Depends(auth_required)):
